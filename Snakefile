...
# In this revision we added some funtions to ensure robust and userfriendly checks
"""
from snakemake.exceptions import WorkflowError

# ---- Config helpers (robust against empty/mistyped config values) ----
def _clean(v):
    """Normalize config values to stripped strings. We treat all crap vlalues/signs as empty."""
    if v is None: # "none" is basically; gtf_directory: null (or ~,t,g,anything except ""), instead of gtf_directory: "".
        return "" # so we make it ""
    # Keep non-strings (e.g. int/bool) but adn remove leading and tailing white spaces
    return str(v).strip()

def _as_yesno(v, key, default=""):
    """Accept yes/no/true/false/1/0; return "yes" or "no" or "" (if default="")."""
    s = _clean(v).lower() # rund funtion " _clean" and remove capital signs
    ...
    if s in ("n","no","false","f","0","off"):
        return "no"
    raise WorkflowError(f'Config "{key}" in the YAML must be yes/no (or true/false/1/0). Got: {v!r}. Sort out your life!')
...
# This does not change the YAML file on disk. It only changes the in-memory config dict Snakemake is using for this run.
EXPECTED_KEYS = [
...
# ---------------------------------------------------------------------------
# End of paramerter checks in the config YAML
...
# By complexity
UMI_FEATURE_COMPLEXITY = config["UMI.feature.complexity"]
UMI_FEATURE_COMPLEXITY_VALUE = config["UMI.feature.complexity.value"]
...
def raise_error():
    ...
    if ANIMAL == "":
        ...
        'MT/Ribo genes from the Ensembl database. Go back to the config file and specify either the name of your specie ("animal") '
...
### RUN QC RENAMING THE KNIT LOCALLY as this is not demanding anyway ###
localrules: rename_knits, #QC_and_Pre_processing  # if renaming the knit locally after the cluster operation, feel free to remove it from localrules
...
# As a safequard for interuption (e.g. ctrl+c, or whatever), the half-generated filtered.gtf file will be removed. This ensures that the snakefile does't skip "Cellranger mkgtf" when pipeline is reactivated.
"""
...
if CELLRANGER_DIRECTORY != "" and MAKE_NEW_REF == "yes":
    rule cellranger_mkgtf:
        ...
        shell:
            ...
            # If rule is interrupted (Ctrl+C, kill, scheduler cancel), delete the temp file.
            cleanup() {{
                rm -f "$tmp"
            }}
            ...
            "{params.cellranger_directory}/cellranger" mkgtf \
                "{input}" "$tmp" {params.mkgtf_options}
    
            # Only create the real output when mkgtf fully succeeded
            mv -f "$tmp" "{output}"
    
            # Success: don’t run cleanup on EXIT anymore
            trap - EXIT
            """
# Original shell line above: {params.cellranger_directory}/cellranger mkgtf {input} {output} {params.mkgtf_options}
...
if CELLRANGER_DIRECTORY != "" and READS_DIRECTORY != "" :
    rule cellranger_count:
        ...
        shell:
            """
            rm -r -f {params.move_to}/{params.sample_numbers}
            ...
            # Safer approuch, to replace "rm -r -f {params.move_to}/{params.sample_numbers}"
...
if SAMPLE_NAMES == "" :
    rule QC_and_Pre_processing:
        ...
        params:
            ...
            UMI_feature_complexity = UMI_FEATURE_COMPLEXITY,
            UMI_feature_complexity_value = UMI_FEATURE_COMPLEXITY_VALUE,
            CPU_cores = CPU_CORES,
            skip_DoubletFinder = SKIP_DOUBLETFINDER,
            knits = lambda wildcards: os.path.join(OUT_DIRECTORY, wildcards.samples),
        conda:
            "sc_sn_RNA_seq_pipeline"
        script:
...
####### TEST CODE ####### 
####### Please do not remove, this is used for debugging previous work ####### 
####### These are required to learn and implement future updates (i have Alzheimer some times, ehehe)  ####### 
